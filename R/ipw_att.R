#' @title The average treatment effect calculated using inverse propensity weighting.
#'
...
#' # Run the function
#' ipwobject <- ipw_att(index=data$unit,y1=data$y,y0=datapre$y,D=data$posttreat,covariates = NULL)
#' # Report the ATT
#' ipwobject$ATT
ipw_att <- function(index,y1, y0, D, covariates, i.weights = NULL, dist.family = stats::gaussian(link = "identity"), alpha = 0.05, conformal_split = 1, weighted_conformal = FALSE){
  #-----------------------------------------------------------------------------
  # D as vector
  D <- as.vector(D)
  # Sample size
  ...
  ipw.att.treat <- w.treat * deltaY
  ipw.att.cont <- w.cont * deltaY

  eta.treat <- mean(ipw.att.treat) / mean(w.treat)
  eta.cont <- mean(ipw.att.cont) / mean(w.cont)
  ...
  for (i in unique(split_index)){
    if (i==0){
      ...
      if (length(Ntrain)<dim(int.cov)[2]){
        ...
        M_full[which(index %in% Ncalib)] <- mean(deltaY[which(index %in% Ntrain)]*i.weights[which(index %in% Ntrain)]*ipw0)/mean(i.weights[which(index %in% Ntrain)]*ipw0)
        CS_full[which(index %in% Ncalib)] <- deltaY[which(index %in% Ncalib)] - mean(deltaY[which(index %in% Ntrain)])
      }
    }
  }
  ...
  return(list(ATT = ipw.att, lci = mean(deltaY[D==1])-CSqu, uci = mean(deltaY[D==1])-CSql, conf.score.weights = CSmat))
