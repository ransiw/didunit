#' Computes aggregated treatment effect parameters
#'
...
compute.aggite <- function(MP,
  ...
  overall.i <- lapply(1,function(h){
    whichi <- which((group <= t) & (t<= (group + max_e)))  ### added last condition to allow for limit on longest period included in att
    whichiN <- id[(group <= t) & (t<= (group + max_e))]
  ...
  overall.att <- unlist(BMisc::getListElement(overall.i, "att"))
  overall.lci <- unlist(BMisc::getListElement(overall.i, "lci"))
  overall.uci <- unlist(BMisc::getListElement(overall.i, "uci"))
  overall.count <- unlist(BMisc::getListElement(overall.i, "count"))

  if (type == "simple"){
    return(AGGITEobj(overall.att = overall.att,
                     overall.se = NULL,
  ...
  if (type == "unit") {
    ...
    # get unit specific ATTs and conformal intervals
    unit.i <- lapply(idlist, function(g){
      whichi <- which( (id == g) & (group <= t) & (t<= (group + max_e)))  ### added last condition to allow for limit on longest period included in att
      whichiN <- id[(id == g) & (group <= t) & (t<= (group + max_e))]
      # calculate the aggregate att and deltaYi
      atti <- att[whichi]
      deltaYi <- deltaY[whichi]
      att.i <- sum(atti*(pg[whichi]/sum(pg[whichi])))
    ...
    unit.att <- unlist(BMisc::getListElement(unit.i, "att"))
    unit.lci <- unlist(BMisc::getListElement(unit.i, "lci"))
    unit.uci <- unlist(BMisc::getListElement(unit.i, "uci"))
    unit.count <- unlist(BMisc::getListElement(unit.i, "count"))
  ...
  if (type == "group") {
    ...
    # get treatment-time group ATT
    group.i <- lapply(glist,function(h){
      whichi <- which((group == h) & (group <= t) & (t<= (group + max_e)))  ### added last condition to allow for limit on longest period included in att
      whichiN <- id[((group == h) & group <= t) & (t<= (group + max_e))]
      # calculate the aggregate att and deltaYi
      atti <- att[whichi]
      deltaYi <- deltaY[whichi]
      att.i <- sum(atti*(pg[whichi]/sum(pg[whichi])))
    ...
    group.att <- unlist(BMisc::getListElement(group.i, "att"))
    group.lci <- unlist(BMisc::getListElement(group.i, "lci"))
    group.uci <- unlist(BMisc::getListElement(group.i, "uci"))
    group.count <- unlist(BMisc::getListElement(group.i, "count"))
  ...
  #-----------------------------------------------------------------------------
  # Compute the custom aggregates
  #-----------------------------------------------------------------------------

  if (type %in% c(customnames)) {

    # get ATTs for each specified category
    custom.i <- lapply(cohortlist,function(h){
      whichi <- which( (ccohort == h) & (group <= t) & (t<= (group + max_e)))
      whichiN <- id[which( (ccohort == h) & (group <= t) & (t<= (group + max_e)))]
      # calculate the aggregate att and deltaYi
      atti <- att[whichi]
      deltaYi <- deltaY[whichi]
      att.i <- sum(atti*(pg[whichi]/sum(pg[whichi])))
    ...
    custom.att <- unlist(BMisc::getListElement(custom.i, "att"))
    custom.lci <- unlist(BMisc::getListElement(custom.i, "lci"))
    custom.uci <- unlist(BMisc::getListElement(custom.i, "uci"))
    custom.count <- unlist(BMisc::getListElement(custom.i, "count"))
  ...
  if (type == "dynamic") {
    ...
    dynamic.i <- lapply(eseq,function(e){
      whichi <- which( (originalt - originalgroup == e) & (include.balanced.gt))
      whichiN <- id[(originalt - originalgroup == e) & (include.balanced.gt)]
      # calculate the aggregate att and deltaYi
      atti <- att[whichi]
      deltaYi <- deltaY[whichi]
      att.i <- sum(atti*(pg[whichi]/sum(pg[whichi])))
    ...
    dynamic.att <- unlist(BMisc::getListElement(dynamic.i, "att"))
    dynamic.lci <- unlist(BMisc::getListElement(dynamic.i, "lci"))
    dynamic.uci <- unlist(BMisc::getListElement(dynamic.i, "uci"))
    dynamic.count <- unlist(BMisc::getListElement(dynamic.i, "count"))
  ...
  if (type == "calendar") {
    ...
    calendar.i <- lapply(calendar.tlist,function(t1){
      whichi <- which( (t == t1) & (group <= t))
      whichiN <- id[which( (t == t1) & (group <= t))]
      # calculate the aggregate att and deltaYi
      atti <- att[whichi]
      deltaYi <- deltaY[whichi]
      att.i <- sum(atti*(pg[whichi]/sum(pg[whichi])))
    ...
    calendar.att <- unlist(BMisc::getListElement(calendar.i, "att"))
    calendar.lci <- unlist(BMisc::getListElement(calendar.i, "lci"))
    calendar.uci <- unlist(BMisc::getListElement(calendar.i, "uci"))
    calendar.count <- unlist(BMisc::getListElement(calendar.i, "count"))
    ...
    return(AGGITEobj(overall.att = overall.att,
                     ...
                     att.egt=calendar.att,
                     se.egt=NULL,
                     lci.egt=calendar.lci,
                     uci.egt=calendar.uci,
                     count.egt=calendar.count,
                     crit.val.egt=NULL,
                     inf.function = NULL,
                     call=call,
                     indep = indep,
