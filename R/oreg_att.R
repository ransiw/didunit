#' @title The average treatment effect calculated using the outcome regression.
#'
...
#' # Run the function
#' oregobject <- oreg_att(index=data$unit,y1=data$y,y0=datapre$y,D=data$posttreat, covariates = NULL)
#' # Report the ATT
#' oregobject$ATT
oreg_att <- function(index,y1, y0, D, covariates, i.weights = NULL, dist.family = stats::gaussian(link = "identity"), alpha = 0.05, conformal_split = 1, weighted_conformal = FALSE){
  #-----------------------------------------------------------------------------
  # D as vector
  D <- as.vector(D)
  # Sample size
  ...
  for (i in unique(split_index)){
    if (i==0){
      ...
      if (length(Ntrain)<dim(int.cov)[2]){
        ...
        M_full[which(index %in% Ncalib)] <- mean(out.delta[D==1]*i.weights[D==1])/mean(i.weights[D==1])
  ...
  return(list(ATT = reg.att, lci = mean(deltaY[D==1])-CSqu, uci = mean(deltaY[D==1])-CSql, conf.score.weights = CSmat))
